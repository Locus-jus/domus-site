...
import { deleteCloudInscription, fetchCloudDebates, fetchCloudInscriptions, seedCloudInscriptions } from "@/lib/supabase-data";
...
export default function InscriptionManager() {
  ...
  const refresh = async () => {
    setLoading(true);
    if (!localStorage.getItem("domus_legacy_inscriptions_migrated")) {
      await seedCloudInscriptions();
      localStorage.setItem("domus_legacy_inscriptions_migrated", "1");
    }
    ...
    <div className="space-y-6">
      ...
      {groups.map(([debateId, registrations]) => (
        <section key={debateId} className="bg-domus-surface border border-domus-border-light rounded-[var(--radius-lg)] overflow-hidden">
          ...
          <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="border-b border-domus-border-light"><th className="p-4 text-xs uppercase text-domus-text-muted">Nome</th><th className="p-4 text-xs uppercase text-domus-text-muted">E-mail</th><th className="p-4 text-xs uppercase text-domus-text-muted">Sociedade</th><th className="p-4 text-xs uppercase text-domus-text-muted">Categoria</th><th className="p-4 text-xs uppercase text-domus-text-muted">Status</th><th className="p-4 text-xs uppercase text-domus-text-muted">Ação</th></tr></thead><tbody>{registrations.map((item) => <tr key={item.id} className="border-b last:border-0 border-domus-border-light"><td className="p-4 text-sm text-domus-text">{item.name}</td><td className="p-4 text-sm text-domus-text-secondary">{item.email}</td><td className="p-4 text-sm text-domus-text-secondary">{item.society}</td><td className="p-4 text-sm text-domus-text-secondary">{item.category}</td><td className="p-4 text-sm text-domus-text-secondary">{item.status}</td><td className="p-4"><button onClick={async () => { deleteStoredInscription(item.id); await deleteCloudInscription(item.id); setItems((current) => current.filter((entry) => entry.id !== item.id)); }} className="text-sm text-red-500 hover:text-red-700">Remover</button></td></tr>)}</tbody></table></div>
        </section>
      ))}
    </div>
  );
